...
*/
	...
	public function hobbies() { 		return $this->data["hobbies"]; }
...
*/
	...
	public function get_applications() {
		/*
			Eerst uitzoeken welke applicaties de gebruiker mag benaderen!
			Dit wordt later geïmplementeerd.
		*/
		/*
			Haal applicatities op uit admin database.
		*/
		$admin_connection = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, ADMIN_DB_NAME) ;
		if (mysqli_connect_errno()) {
			echo "Connect admin failed: " . mysqli_connect_error();
			exit();
		}
		$stmt = $admin_connection->prepare("select APPLICATION_ID, DATABASE_NAME from APPLICATION");
		$stmt->execute();
		$applications = array();
		$stmt->bind_result($application_id, $db_name);
		while ($stmt->fetch()) {
			$applications[$application_id] = $db_name;
		}
		$stmt->close();
...
*/	
	public function from_database($person_id) {
		...
		*/
		...
		foreach($apps as $app_id => $db_name) {
			$app_connection = new mysqli(DB_HOST, DB_USER, DB_PASSWORD, $db_name) ;
                        if (mysqli_connect_errno()) {
                                echo "Connect " . $db_name . "failed: " . mysqli_connect_error();
                                exit();
                        }
			$mbr_query = $app_connection->prepare("
				select Group_GROUP_ID from Group_MEMBERS
				where PERSON_ID = ?");
			$mbr_query->bind_param("i", $person_id);
			$mbr_query->execute();
			$mbr_query_result = $mbr_query->get_result();
			$mbr_query->close();
			while ($mbr_row = $mbr_query_result->fetch_assoc()) {
				$prf_query = $app_connection->prepare("
					select PROFILE_ID, discriminator from PROFILE
					where GROUP_GROUP_ID = ?");
				$prf_query->bind_param("i", $mbr_row["Group_GROUP_ID"]);
				$prf_query->execute();
				$prf_query_result = $prf_query->get_result();
				while($prf_row = $prf_query_result->fetch_assoc()) {
					$this->data["profiles"][$db_name] = array(
						"profile_id" => $prf_row["PROFILE_ID"],
						"discriminator" => $prf_row["discriminator"]
					);
				}
				$prf_query->close();
			}
...
*/
		...
		$stmt = $this->connection->prepare("update PERSON_ADDRESS set 
				...
				NUMBER = ?
				where PERSON_PERSON_ID = ?");
		$ext = trim($this->data["extension"]);
		$lat = trim($this->data["latitude"]);
		$lon = trim($this->data["longtitude"]);
		$num = trim($this->data["number"]);
...
*/
	public function update() {
		...
		$stmt = $this->connection->prepare("update PERSON set 
				...
				UPAS = ?
			where PERSON_ID = ?");
		$calling_name = trim($this->data["calling_name"]);
		$first_name = trim($this->data["firstname"]);
		$prefix = trim($this->data["prefix"]);
	...
	private function store_contacts() {
		...
		foreach($this->data["contacts"] as $contact) {
			...
			$restricted = (isset($contact["restricted"]) ? 1 : 0);
	...
	public function get_application_member_data($person_id) {
		$return = false;
		if (!$person_id) {
			return false;
		}
